...
<div class="max-w-7xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        ...
        <button onclick="openCreateForm()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">+ Add Department</button>
    </div>
...
$(function() {
    table = $('#dataTable').DataTable({
        processing: true, ajax: '{{ route("admin.departments.data") }}',
        columns: [
            { data: 'id' },
            { data: 'name' },
            { data: 'name_bn' },
...
function openCreateForm() { $.get('{{ route("admin.departments.create") }}', function(res) { openModal(res.html); }); }
function openEditForm(id) { $.get('/admin/departments/' + id + '/edit', function(res) { openModal(res.html); }); }
function deleteRecord(id) { confirmDelete('/admin/departments/' + id, function() { table.ajax.reload(); }); }
