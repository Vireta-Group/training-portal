...
<div class="max-w-7xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        ...
        <button onclick="openCreateForm()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">+ Add Batch</button>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table id="dataTable" class="w-full">
            <thead><tr><th>ID</th><th>Project</th><th>Course</th><th>Name</th><th>Shift</th><th>Seats</th><th>Status</th><th>Actions</th></tr></thead>
        </table>
    </div>
</div>
@endsection
...
$(function() {
    table = $('#dataTable').DataTable({
        processing: true, ajax: '{{ route("admin.batches.data") }}',
        columns: [
            { data: 'id' },
...
function openCreateForm() { $.get('{{ route("admin.batches.create") }}', function(res) { openModal(res.html); }); }
function openEditForm(id) { $.get('/admin/batches/' + id + '/edit', function(res) { openModal(res.html); }); }
function deleteRecord(id) { confirmDelete('/admin/batches/' + id, function() { table.ajax.reload(); }); }
