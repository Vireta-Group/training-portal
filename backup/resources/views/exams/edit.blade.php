                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('exams.update', $exam) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $exam->name)" required />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="name_bn" value="Exam Name (Bangla)" />
                            <x-text-input id="name_bn" class="block mt-1 w-full" type="text" name="name_bn" :value="old('name_bn', $exam->name_bn)" />
                        ...
                        <div class="mt-4">
                            ...
                            <select id="type" name="type" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                <option value="model_test" {{ old('type', $exam->type) == 'model_test' ? 'selected' : '' }}>Model Test</option>
                                <option value="retake" {{ old('type', $exam->type) == 'retake' ? 'selected' : '' }}>Retake</option>
                            </select>
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                <x-text-input id="total_marks" class="block mt-1 w-full" type="number" name="total_marks" :value="old('total_marks', $exam->total_marks)" min="1" step="any" required />
                                <x-input-error :messages="$errors->get('total_marks')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="passing_marks" value="Passing Marks *" />
                                <x-text-input id="passing_marks" class="block mt-1 w-full" type="number" name="passing_marks" :value="old('passing_marks', $exam->passing_marks)" min="0" step="any" required />
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" name="description" rows="3">{{ old('description', $exam->description) }}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="status" name="status" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                <option value="pending" {{ old('status', $exam->status ?? 'pending') == 'pending' ? 'selected' : '' }}>Pending</option>
                        ...
                        <div class="flex items-center justify-end mt-6 gap-3">
                            ...
                            <button type="submit" class="btn btn-solid-amber">Update Exam</button>
                        </div>
                    </form>
                </div>
            </div>
